...
extern "C" {
		...
		*/
		...
		/** @brief HID设备拔插事件通知回调函数
			@param vid  设备vid
			@param pid	设备pid
			@param path 设备路径
			@param user_data 用户调用hid_register_device_notification时传入的参数
			@param removal 1--拔出设备  0--插入设备
		*/
		typedef void (*hid_pnp_callback)(unsigned short vid, unsigned short pid, const char* path, int removal, void* user_data);
		
		
		/** @brief 注册HID设备拔插事件通知回调函数.
		 
		*/
		void HID_API_EXPORT HID_API_CALL hid_register_device_notification(hid_pnp_callback fn, void* user_data);


		/** @brief 获取最大的输入报文长度
			@param device A device handle returned from hid_open().
		*/
		int HID_API_EXPORT HID_API_CALL hid_max_input_report_len(hid_device *device);
		#ifdef __cplusplus
}
#endif
